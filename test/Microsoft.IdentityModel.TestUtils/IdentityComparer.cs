...
namespace Microsoft.IdentityModel.TestUtils
{
    public class IdentityComparer
    {
        ...
        public static bool AreJsonWebKeysEqual(object obj1, object obj2, CompareContext context)
        {
            var localContext = new CompareContext(context) { IgnoreType = true };
            if (!ContinueCheckingEquality(obj1, obj2, localContext))
                return context.Merge(localContext);

            Type jsonWebKeyType1 = obj1.GetType();
            var jsonWebKeyPropertyInfos1 = jsonWebKeyType1.GetProperties(BindingFlags.Public | BindingFlags.Instance);

            Type jsonWebKeyType2 = obj2.GetType();
            var jsonWebKeyPropertyInfos2 = jsonWebKeyType2.GetProperties(BindingFlags.Public | BindingFlags.Instance);

            foreach (PropertyInfo jsonWebKeyPropertyInfo1 in jsonWebKeyPropertyInfos1)
            {
                bool skipProperty = false;
                if (context.PropertiesToIgnoreWhenComparing != null && context.PropertiesToIgnoreWhenComparing.TryGetValue(jsonWebKeyType1, out List<string> propertiesToIgnore))
                {
                    foreach (var val in propertiesToIgnore)
                        if (string.Equals(val, jsonWebKeyPropertyInfo1.Name, StringComparison.OrdinalIgnoreCase))
                        {
                            skipProperty = true;
                            break;
                        }
                ...
                foreach (PropertyInfo jsonWebKeyPropertyInfo2 in jsonWebKeyPropertyInfos2)
                {
                    if (jsonWebKeyPropertyInfo2.Name == jsonWebKeyPropertyInfo1.Name)
                        propertyInfoFound = jsonWebKeyPropertyInfo2;
                }

                if (propertyInfoFound == null)
                {
                    localContext.AddDiff($"jsonWebKeyNet8 property not found: {jsonWebKeyPropertyInfo1.Name}");
                    continue;
                }

                if (jsonWebKeyPropertyInfo1.GetMethod != null)
                {
                    var propertyContext = new CompareContext(context);

                    object val1 = jsonWebKeyPropertyInfo1.GetValue(obj1, null);
                    object val2 = propertyInfoFound.GetValue(obj2, null);
                    if ((val1 == null) && (val2 == null))
                        continue;

                    if ((val1 == null) || (val2 == null))
                    {
                        propertyContext.Diffs.Add($"{jsonWebKeyPropertyInfo1.Name}:");
